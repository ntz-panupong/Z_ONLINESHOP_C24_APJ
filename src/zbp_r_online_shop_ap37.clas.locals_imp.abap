CLASS lsc_zbp_r_online_shop_c37_apj DEFINITION INHERITING FROM cl_abap_behavior_saver.
  PROTECTED SECTION.
    METHODS save_modified REDEFINITION.
ENDCLASS.

CLASS lsc_zbp_r_online_shop_c37_apj IMPLEMENTATION.

  METHOD save_modified.
    DATA : lt_online_shop_as TYPE STANDARD TABLE OF zshop_as_AP37,
           ls_online_shop_as TYPE                   zshop_as_AP37.
    IF zbp_r_online_shop_AP37=>cv_pr_mapped-purchaserequisition IS NOT INITIAL.
      LOOP AT zbp_r_online_shop_AP37=>cv_pr_mapped-purchaserequisition ASSIGNING FIELD-SYMBOL(<fs_pr_mapped>).
    ...
    IF create-online_shop IS NOT INITIAL.
      ...
      INSERT zshop_as_AP37 FROM TABLE @lt_online_shop_as.
    ENDIF.
  ENDMETHOD.
ENDCLASS.

CLASS lhc_zbp_r_online_shop_C37_APJ  DEFINITION INHERITING FROM cl_abap_behavior_handler.
  PRIVATE SECTION.
...
CLASS lhc_zbp_r_online_shop_C37_APJ  IMPLEMENTATION.
...
TRY.
      ...
      WITH VALUE #( ( %cid_ref = 'My%ItemCID_1'
                      %target  = VALUE #( (
                                          ...
                                          textobjecttype = 'B01'
                                          language       = 'E'
                                          plainlongtext  = 'item text created from PAAS API C37 APJ - Workshop prep.'
                                        ) (
                                          %cid           = 'My%CCT_2'
                                          textobjecttype = 'B02'
                                          language       = 'E'
                                          plainlongtext  = 'item2 text created from PAAS API C37 APJ'
                                        ) )
              )   )
      ...
      zbp_r_online_shop_AP37=>cv_pr_mapped = ls_pr_mapped.
...
DATA:
      online_shops TYPE TABLE FOR UPDATE zr_online_shop_AP37,
      online_shop  TYPE STRUCTURE FOR UPDATE zr_online_shop_AP37.

    READ ENTITIES OF zr_online_shop_AP37 IN LOCAL MODE
       ENTITY Online_Shop
        ALL FIELDS
    ...
    LOOP AT lt_online_shop_result INTO DATA(online_shop_read).
      ...
      ENDIF.
    ENDLOOP.
    MODIFY ENTITIES OF zr_online_shop_AP37 IN LOCAL MODE
    ENTITY Online_Shop UPDATE SET FIELDS WITH online_shops
    MAPPED   DATA(ls_mapped_modify)
    FAILED   DATA(lt_failed_modify)
    REPORTED DATA(lt_reported_modify).

    DATA: lt_create_pr_imp TYPE TABLE FOR ACTION IMPORT  zr_online_shop_AP37~create_pr,
